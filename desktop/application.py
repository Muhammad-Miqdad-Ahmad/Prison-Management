...
class MainApp(QMainWindow):
    ...
    def init_guard_dashboard(self):
        ...
        self.submit_complaint_button = QPushButton("Submit Complaint")
        self.submit_complaint_button.setStyleSheet(self.button_style)
        self.submit_complaint_button.clicked.connect(self.show_complaint_view)
        ...
        layout.addWidget(self.submit_complaint_button)
    ...
    def init_complaint_view(self):
        ...
        back_button = QPushButton("Back")
        back_button.setStyleSheet(self.button_style)
        back_button.clicked.connect(self.show_prisoner_dashboard)
    ...
    def show_upcoming_visitations(self):
        try:
            ...
            self.visitation_table.setHorizontalHeaderLabels(["Slot ID", "Slot Time", "Reservation Time", "Visitor ID", "Visit Date"])
            for row_idx, visitation in enumerate(visitations):
                ...
                self.visitation_table.setItem(row_idx, 1, QTableWidgetItem(visitation['slot_time']))
                self.visitation_table.setItem(row_idx, 2, QTableWidgetItem(visitation['reservation_time']))
                self.visitation_table.setItem(row_idx, 3, QTableWidgetItem(str(visitation['visitor_id'])))
                self.visitation_table.setItem(row_idx, 4, QTableWidgetItem(visitation['visit_date']))
    ...
    def view_prisoner_records(self):
        try:
            ...
            for row_idx, prisoner in enumerate(prisoners):
                ...
                self.prisoner_table.setItem(row_idx, 1, QTableWidgetItem(prisoner['person']['first_name'] + " " + prisoner['person']['last_name']))
    ...
    def view_visitation_details(self):
        try:
            ...
            for row_idx, visitation in enumerate(visitations):
                self.visitation_table.setItem(row_idx, 0, QTableWidgetItem(str(visitation['visiting_id'])))
                self.visitation_table.setItem(row_idx, 1, QTableWidgetItem(str(visitation['prision_id'])))
                self.visitation_table.setItem(row_idx, 2, QTableWidgetItem(visitation['day_of_week']))
                self.visitation_table.setItem(row_idx, 3, QTableWidgetItem(visitation['start_time']))
                self.visitation_table.setItem(row_idx, 4, QTableWidgetItem(visitation['end_time']))
            self.stacked_widget.addWidget(self.visitation_table)
            self.stacked_widget.setCurrentWidget(self.visitation_table)
        except Exception as e:
